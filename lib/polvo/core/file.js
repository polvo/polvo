...
(function() {
  var Coffeescript, File, Jade, Stylus, debug, error, fs, fsu, log, path, warn, _ref,
  ...
  module.exports = File = (function() {
    File.EXTENSIONS = [Coffeescript.EXT, Jade.EXT, Stylus.EXT];

    File.COMPILERS = [Coffeescript, Jade, Stylus];
    ...
    File.prototype.compile_to_str = function(after_compile, exclude_anonymous_requires) {
      var _this = this;

      return this.compiler.compile(this, function(js, map, src) {
        js = _this.inject_dependencies(js, exclude_anonymous_requires);
        if (exclude_anonymous_requires) {
          js = _this.exclude_anonymous_requires(js);
        }
        return typeof after_compile === "function" ? after_compile(js, map, src) : void 0;
      });
    };

    File.prototype.compile_to_disk = function() {
      var now,
        _this = this;
      ...
      return this.compile_to_str(function(js, src_map, src) {
        ...
        msg = '✓ Compiled'.bold;
        return log(("[" + now + "] " + msg + " " + _this.out.relative_path).green);
      });
    };
    ...
    File.prototype.inject_dependencies = function(js_code, exclude_anonymous_requires) {
      var dep, paths, replace, search, _i, _len, _ref1;

      if (exclude_anonymous_requires == null) {
        exclude_anonymous_requires = false;
      }
      ...
      if (this.dependencies.length) {
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          dep = _ref1[_i];
          if (exclude_anonymous_requires && dep.incompatible) {
            continue;
          } else {
            paths.push(dep.id);
          }
    ...
    File.prototype.exclude_anonymous_requires = function(code) {
      var reg;
