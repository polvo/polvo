...
var File, MicroEvent, argv, config, debug, dirs, error, fs, info, notify, path, plugins, prefix, scan, sufix, warn, _, _ref,
...
module.exports = File = (function(_super) {
  ...
  File.prototype.initialized = false;
  ...
  File.prototype.outputpath = null;
  ...
  File.prototype.init = function() {
    if (this.is_partial && !this.initialized) {
  ...
  File.prototype.compile = function(done) {
    ...
    return this.compiler.compile(this.filepath, this.raw, !argv.release, function(err) {
      ...
      var input, relative, _i, _len, _ref1;
      _this.compiled = compiled;
      _this.source_map = source_map;
      if (_this.compiler.type === 'template' && (config.output.html != null)) {
        _ref1 = config.input;
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          input = _ref1[_i];
          if (~_this.filepath.indexOf(input)) {
            relative = _this.filepath.replace(input, '');
            break;
          }
        }
        _this.outputpath = path.join(config.output.html, relative);
        _this.outputpath = _this.outputpath.replace(_this.compiler.ext, '.html');
        _this.outputpath = path.join(dirs.pwd, _this.outputpath);
        if (!argv.release) {
          notify(_this.outputpath);
        }
        fs.writeFileSync(_this.outputpath, _this.compiled);
      }
      return done();
    });
  };
