...
var File, MicroEvent, argv, debug, dirs, error, fs, info, path, plugins, prefix, scan, sufix, warn, _, _ref,
...
module.exports = File = (function(_super) {
  ...
  File.prototype.refresh = function() {
    ...
    return this.compile(function() {
      if (_this.compiler.type === 'script') {
        _this.parse_conditionals();
      }
      _this.scan_deps();
      _this.make_aliases();
      _this.wrap();
  ...
  File.prototype.parse_conditionals = function() {
    var after, before, buffer, reg, res, _results;
    reg = /\/\*\s*polvo:if([\s\S]*?)\/\*\s*polvo:fi\s*\*\//g;
    buffer = [];
    _results = [];
    while ((res = reg.exec(this.compiled)) != null) {
      before = res[0];
      after = this.parse_conditional_block(before);
      _results.push(this.compiled = this.compiled.replace(before, after));
    }
    return _results;
  };

  File.prototype.parse_conditional_block = function(block) {
    var buffer, capturing, cond, key, line, passed, value, _i, _len, _ref1, _ref2;
    buffer = '';
    passed = 0;
    capturing = false;
    _ref1 = block.split('\n');
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      ...
      if (/polvo:(if|elif)/.test(line)) {
        cond = line.match(/(\w+)=(\w+)/);
        _ref2 = cond.slice(1), key = _ref2[0], value = _ref2[1];
