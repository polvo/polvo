...
var argv, auto_reload, build_js_split, cli, compilers, config, debug, dirs, error, files, filesize, fs, fsu, get_split_base_dir, helpers_path, info, io_path, loader, loader_path, log_compiled, logger, minify, path, prefix, reloader_path, server, show, source_maps_header, sourcemaps, split_loader, split_loader_path, sufix, warn, _;
...
exports.build_js = function(notify) {
  ...
  server.reload('js');
  if (notify) {
    return exports.notify(config.output.js);
  }
};
