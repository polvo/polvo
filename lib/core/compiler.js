...
exports.build_js = function(notify) {
  var all, base64_maps, base_folder, boot, buffer, comp, comp_name, each, helper, helpers, i, js, k, linesnum, merged, offset, p, relative_paths, split_paths, src_maps, start, tmp, v, _i, _j, _k, _len, _len1, _len2, _ref;
  ...
  if (argv.split) {
    ...
    for (i = _k = 0, _len2 = relative_paths.length; _k < _len2; i = ++_k) {
      p = relative_paths[i];
      base_folder = (config != null ? (_ref = config.server) != null ? _ref.root : void 0 : void 0) || path.dirname(config.output.js);
