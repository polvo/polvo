...
var argv, auto_reload, build_js_split, compilers, config, dirs, files, fs, fsu, get_split_base_dir, helpers_path, io_path, loader, loader_path, minify, notify, path, prefix, reloader_path, server, source_maps_header, sourcemaps, split_loader, split_loader_path, sufix, _;
...
exports.release = function(done) {
  var after, file, htmls, js, jss, pending, uncompressed, _i, _j, _k, _len, _len1, _len2;
  jss = exports.build_js();
  exports.build_css();
  htmls = _.filter(files.files, {
    type: 'template',
    is_partial: false
  });
  pending = 0;
  ...
  for (_k = 0, _len2 = htmls.length; _k < _len2; _k++) {
    file = htmls[_k];
    pending++;
  ...
  notify(js, after);
  return notify(config.output.css, after);
};

exports.build_js = function(_notify) {
...
exports.build_css = function(_notify) {
  ...
  server.reload('css');
  if (_notify) {
    return notify(config.output.css);
  }
};
...
build_js_split = function(files, _notify) {
