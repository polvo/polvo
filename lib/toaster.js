...
(function() {
  ...
  __t('toaster').Toast = (function() {
    ...
    Toast.prototype.toast = function(srcpath, params) {
      var alias, builder, config, debug, dir, folder, item, _i, _len, _ref, _ref1, _ref2, _ref3;
      if (params == null) {
        params = {};
      }
      ...
      config = {
        ...
        src_folders: [],
        files: [],
        exclude: (_ref = params.exclude) != null ? _ref : [],
        bare: (_ref1 = params.bare) != null ? _ref1 : true,
  ...
  __t('toaster.core').Script = (function() {
    ...
    Script.prototype.getinfo = function(declare_ns) {
      var baseclass, decl, dep, deps, klass, match, name, require_reg_all, require_reg_one, rgx, rgx_ext, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _ref3;
      if (declare_ns == null) {
        declare_ns = true;
      }
      ...
      if (this.raw.match(rgx) != null) {
        if (this.namespace !== "" && this.builder.packaging) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            if ((_ref1 = this.classname) == null) {
              this.classname = name;
            }
            this.extending = decl.match(/(\s*extends\s*[^\s]+$)/m);
            if (this.extending) {
              this.extending = this.extending[0];
            }
            this.extending || (this.extending = "");
          }
        }
        this.classname = (this.raw.match(/class\s([^\s]+)/))[1].split('.').pop();
        this.classpath = "" + this.namespace + "." + this.classname;
        _ref3 = (_ref2 = this.raw.match(rgx_ext)) != null ? _ref2 : [];
    ...
    Script.prototype.inject_definitions = function() {
      var def, dep, deps_args, deps_path, exp, identation, idented, match_identation, ns_rest, ns_root, root, _i, _len, _ref;
      deps_path = '';
      deps_args = '';
      ...
      ns_root = (this.namespace.match(/^([^\.]+)/))[1];
      ns_rest = this.namespace.replace("" + ns_root + ".", "");
      if (this.builder.nature.browser) {
      ...
      } else if (this.builder.nature.node) {
        root = "" + ns_root + " = {}";
        exp = "exports." + ns_root + " = {}";
        this.raw = "{toast} = require './toaster'\n";
        this.raw += "__e = toast '" + ns_rest + "', " + root + ", " + exp + ", [ " + deps + " ]\n";
        return this.raw += this.backup.replace(/(class\s)/g, "__e." + this.classname + " = $1");
      }
    };
  ...
  __t('toaster').Cli = (function() {
    ...
    function Cli() {
      ...
      this.argv = (this.opts = optimist.usage(usage).alias('n', 'new').describe('n', "Scaffold a very basic new App").alias('i', 'init').describe('i', "Create a config (toaster.coffee) file").alias('w', 'watch').describe('w', "Start watching/compilin in the given mode.").alias('c', 'compile').describe('c', "Compile in release mode (all js files merged).").alias('d', 'debug').describe('d', 'Compile in debug mode (compile js files individually)').alias('a', 'autorun').describe('a', 'Execute the script in node.js after compilation').alias('j', 'config').string('j').describe('j', "Config file formatted as a json-string.").alias('f', 'config-file').string('f').describe('f', "Path to a different config file.").alias('v', 'version').describe('v', '').alias('h', 'help').describe('h', '')).argv;
    }
  ...
  __t('toaster.core').Builder = (function() {
    ...
    Builder.prototype.build = function(header_code, footer_code) {
      var args, ast, contents, i, now, root_ns, _i, _ref1;
      if (header_code == null) {
        header_code = "";
      }
      if (footer_code == null) {
      ...
      if (this.cli.argv.c) {
        this.compile_all();
        root_ns = this.build_root_namespaces();
        contents = [];
    ...
    Builder.prototype.build_root_namespaces = function() {
    ...
    Builder.prototype.build_ns_tree = function(tree, folderpath) {
      ...
      for (_i = 0, _len = folders.length; _i < _len; _i++) {
        ...
        if (include) {
          _results.push(tree[folder.match(/[^\/\\]+$/m)] = {});
        } else {
    ...
    Builder.prototype.compile = function() {
      var file, msg, output, _i, _len, _ref1;
      _ref1 = this.files;
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        file = _ref1[_i];
        try {
      ...
      output = ((function() {
        ...
        for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
          file = _ref2[_j];
          _results.push(file.defined_raw);
        }
        return _results;
      }).call(this)).join("\n");
    ...
    Builder.prototype.compile_all = function() {
      var absolute_path, ast, compiled, file, files, find, folder_path, index, relative_path, release_path, _i, _len, _ref1;
      release_path = path.dirname(this.release);
      ...
      for (index = _i = 0, _len = _ref1.length; _i < _len; index = ++_i) {
        ...
        try {
          compiled = cs.compile(file.raw, {
            bare: this.bare
          });
          if (this.minify && this.nature.browser) {
  ...
  exports.Toaster = Toaster = (function() {
    ...
    function Toaster(basedir, options, skip_initial_build) {
      ...
      if (this.cli.argv.v) {
        ...
        msg += "\t-ca, -wca, -cda, -wcda";
        error(msg);
      } else if (this.cli.argv.c || this.cli.argv.d) {
        this.toast = new toaster.Toast(this);
