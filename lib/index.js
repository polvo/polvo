...
/**
 * Module constructor
 * @param base_dir {String} Base dir to consider, if not informed, `cwd` is used
 * @return {Application} New application instance
 */
module.exports = function(base_dir) {
  return new Application(base_dir);
};
 ...
 */
function Application(base_dir) {

  /**
   * Holds all envs configs
   * @type {Object}
   */
  this.base_dir = path.resolve(base_dir || process.cwd());;
...
function bulk_apply(app, envs, method, methods) {
  return function(val) {
    envs.forEach(function(e){
      (app.envs[e] = app.envs[e] || env(app, e))[method](val);
    });
    return methods;
  };
}
...
function config_chain(app, envs) {
  ...
  methods.inherit = function(from) {
    ...
    if(!(source = app.envs[from]))
      throw new Error('Cannot inherit from '+ from +', env not found');

    envs.forEach(function(e){
      var into = app.envs[e] || (app.envs[e] = env(app, e));
